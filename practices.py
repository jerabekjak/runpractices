...
class Practices:
    ...
    def __run_scenario(self, tabline, counter):
        for i in self.__c_params:
            for j in self.__c_params:
                scenariocode = self.__scenariocode(self.__pars_name[i], 
                            self.__pars_name[j], tabline, counter)
                outdir = self.__make_outdir(scenariocode)
                ok = self.__h1d.prepare_project(outdir, tabline, i, j)
                if (ok) : 
    ...
    def __scenariocode(self, parname1, parname2, tl, counter):
        return("{}-{}-{}-{}-{}-{}-{}-{}".format(tl[0].strip().zfill(2),
        tl[1].strip(),tl[2].strip(),tl[3].strip(),tl[4].strip(),parname1,parname2,str(counter).zfill(2)))
